...
It also contains benchmarks of the access times of different data cube access patterns from different programming languages. 
...
The size of the dataset can be described spatially, going from a local dataset like a single satellite acquisition to a regional dataset to a global harmonised dataset. 
...
For data formats a higher cloud readiness indicates, that cloud datasets can be accessed partially, i.e. a dataset which is provided in an object store in the cloud does not need to be downloaded in full to access only a small subset of the data.
...
# Governance, Funding and Sustainability of Geospatial Open Source Projects

Open Source software is dependent on a developer community which has the time and capability to make relevant changes to the software. 
In this section we look at the governance, funding and developer community size of different geospatial open source communities. 

## Governance of Geospatial open source communities

The governance of geospatial software projects varies between the different communities and the degree of governance also depends on the size of the developer and user communities with larger communities having a more organized governance approach. 
Some repositories have no governance guidelines specified while some of the projects which are driven mainly by a single main developer have specified that this developer or a small group of repository owners make the final decision. 
Larger projects or overall ecosystems have selected a steering committee or project committee for final decisions. Examples for this kind of governance are  QGIS, Open Data Cube or the JuliaGeo ecosystem.
These steering committees have a size of 7 to 12 persons.

## Funding of Geospatial Open Source Projects

The funding of geospatial open source software is coming from very different sources. 
Most projects don't state their funding source which might be an indicator that developers are doing it in their free time or sometimes on their work time because they need the project for their day to day work.
Some projects state that they are mainly developed by people in specific institutes or firms.
Some of the software development is funded via scientific project work where the software is developed either as a specific deliverable or as a needed tool for the scientific outcome. 
Larger projects like GeoServer, GDAL and QGIS have funding via commercial sponsors or in the case of QGIS via membership fees. 



## Index of software contributions

We developed the Open Source Contributor Index to indicate the contributor base of an open source software project. The OSS contributor index is the number of contributors n with at least n commits to a given repository. 
This is coarse indicator of the contributor size and can determine whether the project is a single developer endeavour, a dedicated small group or a very large contributor base. 
The index should not be compared between repositories too strictly, because the index also depends on the overall number of commits to the repository.
An index below 1 shows that the project is only developed by a single developer. Below 10 means, that the development is split between a dedicated group of people and above 10 there is a significant number of contributors.
This index correlates strongly with the number of stars on github as a measure of popularity.
A small contributor index with a high number of stars shows a high risk of a popular project which is dependent on a very small number of contributors.
 
![*Open Source contributions index* The figure shows the open source contribution index on the y axis and the number of github stars on the x-axis. ](h_index_vs_stars.png)

### A few notes of AI-readiness of geospatial data

Since the start of this overview of emerging technologies for large-scale geospatial analysis, the largest trend in data analysis has been the rise of Artificial Intelligence (AI) and Machine Learning (ML). AI is now present in many areas of scientific data analysis, including geospatial data. In this context, the term "AI-readiness" was introduced to describe the suitability of geospatial data and tools for AI applications. 

Since AI is an overarching term that includes many different techniques and methods, it is difficult to give a precise definition of AI-readiness. Depending on the specific method and use case, data access patterns and computational requirements can vary significantly. In terms of data management, the requirements and tradeoffs for AI applications are similar to those for traditional data analysis. For example, when working with large time series data to train LSTM-like models, efficient access to temporal data is crucial. On the other hand, when training convolutional neural networks (CNNs) on image data, efficient spatial access patterns are more important. This is a similar situation as with traditional geospatial analysis, where different algorithms may require different data access patterns.

In the end the data scientist needs to understand the requirements of the AI method they want to use and choose the right data format and tool accordingly. For this reason, we believe that the overview provided in this document is also useful for AI applications, as it helps to understand the capabilities and limitations of different data formats and tools for geospatial analysis.
...
## Overviews and pyramids for analysis and visualisation of large scale raster data

Overview (also called pyramids) for geospatial data are precomputed,
downsampled copies of the original raster at multiple resolutions that dramatically speed up interactive visualisation.
By storing several lower-resolution representations inside the file (or alongside it),
viewers and mapping libraries can request an appropriate level for the display scale instead of resampling the full-resolution data on every frame, reducing CPU, memory, and data loading.
Overviews can be constructed with different  resampling methods (nearest, average, cubic) chosen to balance sharpness and artifact suppression.
For web maps and large datasets, well-built overviews plus a suitable tile cache or cloud-optimized GeoTIFF layout yield smooth pan/zoom performance and lower server costs.

## CF Conventions

The CF  (Climate and Forecast) Metadata conventions are a set of rules to describe which metadata are necessary to describe a dataset. 
The CF conventions have been developed on top of the NetCDF format. 
The aim of the CF conventions is to set the metadata to provide a definitive description of various datasets, including, what the variable represents, the spatial and temporal extent. 
...
It allows for efficient subsetting but it cannot be accessed multi-threaded, but supports multi-process read/write operations through MPI.
...
It can handle different chunks and there are multiple extensions of the data format for different use cases. It can handle compression. 
It is suitable for datasets that are larger than RAM. The computations on these datasets should be parallizable.
...
The R package [`stars`](https://r-spatial.github.io/stars/) is for handling and analysing raster and vector data cubes, with an arbitrary number of dimensions. It uses the GDAL C++ Multidimensional Array interface (through the R package [`sf`](https://r-spatial.github.io/sf/)) to read and write raster or vector data cubes, or sections of them (slices, limiting dimension ranges, or strided reads: reading dimensions at lower resolution)
...
It is currently free, because Google sees it more as a marketing tool. The algorithms that you want to compute on Google Earth Engine have to be written in a strange sublanguage of JavaScript and you cannot bring your own already working script. 
...
The data might be available on other cloud providers, but the algorithms have to be recoded, because GEE uses a special subset of Python or JavaScript.
The Google Earth Engine is used for example to provide the global forest cover change map.

## CDSE

The CDSE is the Copernicus Data Space Ecosystem and provides free and instant access to data and services from Copernicus Sentinel missions which can be used for observation and analysis. The CDSE replaces the former Data and Information Access Services (DIAS) in providing a wide range of data and information on Earth's land, oceans and atmosphere. 

Besides access to the data, the CDSE also includes data processing tools and different data access possibilities, for example via the Copernicus Browser or by creating an account for a better comprehensive exploration experience. The portal also contains dashboard functionalities, use cases and tutorials, and has the option to use cloud-based tools such as JupyterLab and openEO to analyse data without previous download. 
...
They also deploy this software stack as a Pangeo environment in different cloud computing facilities.  


## LexCube

LexCube is a software for interactively visualizing three-dimensional
data cubes, available as a website and a Python package.
The tool allows to interactively visualize and explore
large scale Earth system science datasets as 3D data cubes.
For the visualization the data is saved as tiles at various
levels-of-detail,
which can be pre-generated so that users will experience minimal latency
while interacting with the data.
LexCube was initially developed as a [NFDI4Earth
Pilot](https://zenodo.org/records/7822389)

## PyramidScheme.jl

[PyramidScheme.jl](https://github.com/JuliaDataCubes/PyramidScheme.jl) is a Julia package to prepare, visualise and analyse large raster datasets. 
It allows to compute spatial overviews for given zarr raster datasets.
These overviews can then be used to interactively visualize the datasets with very low latency.
Given multiple datasets with a pyramid structure the data analysis can be done on the pyramids lazily so that only the data is loaded and computed that is currently needed for the visualisation. 
...
Contrary to other global grids like the UTM grid, e.g., used in Sentinel-2 products, each location is assigned to exactly one cell identifier at each given spatial refinement level.
This eliminates tile overlaps and saves up to 33% of memory that needs to be stored, downloaded, and processed [(Bauer-Marschallinger and Falkner, 2023)](https://doi.org/10.1016/j.isprsjprs.2023.07.015).
DGGS has been used in ESA's Soil Moisture and Ocean Salinity mission, multi-scale flood mapping, and the integration of raster and vector data, among others.
Previous efforts focused on building grids and tools to convert between DGGS cell identifiers and geographical coordinates, leading to tools like Google S2, Uber H3, and HEALPix from NASA.
Projects like [DGGAL](https://dggal.org/) and [GeoPlegma](https://github.com/GeoPlegma/GeoPlegma) provide libraries to use different DGGS in a uniform way.
DGGS is not only about a different geocoding scheme but also about data structures and their indices to store and retrieve areas of interest efficiently.
For example, [Vila et al. 2025](https://lps25.esa.int/lps25-presentations/presentations/1169/_1169.pdf) converted 1 year of Belgium's Sentinel-2 data into the columnar ClickHouse database.
[DGGS.jl](https://github.com/danlooo/DGGS.jl) uses Zarr and a multidimensional spatial index for more efficient handling of bounding boxes in dense raster data.
...
Figure 2 shows the comparison of the access times for the different data formats.
...
![*Comparison of access times for NetCDF4, Zarr and TileDB data formats* The numbers on the x-axis describe different query types going from a single time point for a single pixel location up to a year long time series for a large spatial extent.](year_2_24_50GB.png)



\includepdf[pages=-, addtotoc={1, section, 1, Introduction to Raster Analysis in Julia, rasjulia}]{Intro_Raster_Data_Analysis_ENG_Julia.pdf}
\includepdf[pages=-, addtotoc={1, section, 1, Introduction to Raster Analysis in Python, raspython}]{Intro_Raster_Data_Analysis_ENG_Python.pdf}
\includepdf[pages=-, addtotoc={1, section, 1, Introduction to Raster Analysis in R, rasR}]{Intro_Raster_Data_Analysis_ENG_R.pdf}
\includepdf[pages=-, addtotoc={1, section, 1, Introduction to Vector Datacubes in Python and R, vecdata}]{intro_vectordatacubes.pdf}
