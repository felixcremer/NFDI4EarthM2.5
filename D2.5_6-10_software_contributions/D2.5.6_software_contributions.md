...
One remaining in dealing with multi-modal TB to PB-scale geospatial data is the variety of Coordinate Reference systems used for these datasets. Because
of the size of the dataset, it is infeasible to reproject the data to a common CRS. Therefore we developed a protoype Julia package SphericalSpatialTrees.jl that provides methods for efficient spatial indexing and searching of geospatial data in different coordinate reference systems. The package is based on a spherical tree data structure that allows tree queries independent of the CRS used. It provides methods for nearest neighbor search, range queries, and lazy reprojections of huge datasets. It is designed to work with large datasets that do not fit into memory and can be used in conjunction with other JuliaGeo packages for efficient geospatial data analysis. The reprojection is fast enough to allow interactive visualisation of large datasets in different CRS.

The package currently supports different CRS used for global high-resolution datasets, including EPSG:4326, WebMercator, Equi7, UTM, and ISEA. There is an extension for the package to work with Pyramids.jl to lazily reproject every layer of the pyramid. The package is available as open source software at https://github.com/meggart/SphericalSpatialTrees.jl an still under active development. 
